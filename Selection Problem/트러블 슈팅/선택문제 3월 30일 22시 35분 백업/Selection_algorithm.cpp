...
int Selection(int* arr, int left, int right, int k, int* p_comparisonCount)
{
    ...
    SwapArray(arr, pivot, left); // 피벗을 맨 앞으로
    int low = left + 1;
    int high = right;
    ...
    SwapArray(arr, left, high); // 피벗 제자리로 이동

    int pivotRank = high - left + 1; // 몇 번째 작은 값인지

    if (k == pivotRank)
        return arr[high];
    else if (k < pivotRank)
        return Selection(arr, left, high - 1, k, p_comparisonCount);
    else
        return Selection(arr, high + 1, right, k - pivotRank, p_comparisonCount);
}
...
void MedianQuickSort(int* arr, int start, int end, int* p_comparisonCount) {
    ...
    MedianQuickSort(arr, start, lt - 1, p_comparisonCount);
    MedianQuickSort(arr, gt + 1, end, p_comparisonCount);
}
...
int main() {
    ...
    kth_smallest_value = Selection(arr, 0, arr_length, k, &comparisonCount);
    kth_smallest_value2 = arr2[k];
